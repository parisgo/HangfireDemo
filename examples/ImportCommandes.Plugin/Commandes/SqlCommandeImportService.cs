...
public sealed class SqlCommandeImportService(
    ...
    ILogger logger) : ICommandeImportService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        AddParameter(command, "@BatchId", MaximumBatchIdLength, request.BatchId);
        AddParameter(command, "@RequestedBy", MaximumRequestedByLength, Truncate(request.RequestedBy, MaximumRequestedByLength));
        AddParameter(command, "@HangfireJobId", MaximumJobIdLength, request.JobId is null ? DBNull.Value : Truncate(request.JobId, MaximumJobIdLength));
    ...
    private static void AddParameter(DbCommand command, string name, int size, object value)
    {
